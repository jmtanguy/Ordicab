...
function hit(itemId: string, charStart: number, score: number): SemanticSearchHit {
  return { itemId, charStart, charEnd: charStart + 10, score, snippet: '' }
}
...
describe('mergeHybridHits', () => {
  it('puts keyword hits first and tags lanes', () => {
    ...
    expect(merged.map((m) => m.hit.itemId)).toEqual(['a', 'b'])
  })
  ...
  it('drops a semantic hit on a document already represented by a keyword hit', () => {
    ...
    expect(merged.map((m) => `${m.matchKind}:${m.hit.itemId}`)).toEqual(['keyword:a', 'semantic:b'])
  })
